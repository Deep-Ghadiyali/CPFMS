...
const getGujNum = (num) => {
    ...
    {
        if (e2gMap.hasOwnProperty(numStr.charAt(i)))
            gujNum += e2gMap[numStr.charAt(i)];
        else
            gujNum += numStr.charAt(i);
    }
    return gujNum;
}

const getGujDate = (date) => {
    var d = new Date(date);
    var dd = date.getDate().toString();
    var mm = (date.getMonth() + 1).toString();
    var yyyy = (date.getFullYear()).toString();
    ...
    if (mm.length == 1)
        mm = "0" + mm;
    var fdate = getGujNum(dd) + "/" + getGujNum(mm) + "/" + getGujNum(yyyy);
    return fdate;
}
            ...
            if (data.select_all == '1') {
                ...
                sql = `
                    ...
                    FROM Account_Balance
                        LEFT JOIN Account_Head
                            ON Account_Head.account_id = Account_Balance.account_id
                    WHERE Account_Head.account_id IN (?)
                    GROUP BY Account_Balance.account_id;
                    SELECT
                        Ledger.account_id AS aid,
                        (IFNULL(SUM(Ledger.cr_amount),0) - IFNULL(SUM(Ledger.dr_amount),0)) AS op2
...
router.get('/kotha', middleware.loggedin_as_superuser, (req, res) => {
    var data = req.query;
    var flag = true;
    var sql, sql_arr = [];
    if (data.from_date && data.to_date)
    {
        if (data.account_id)
        {
            if (data.select_all == '1')
            {
                sql_arr = [data.account_id, data.account_id, data.from_date, data.to_date, data.account_id];
                sql = `
                    SET NAMES UTF8;
                    SELECT
                        Account_Head.account_id AS sos_id,
                        Account_Head.account_name_guj AS sos_name,
                        Village.village_name_guj AS village,
                        Taluka.taluka_name_guj AS taluka,
                        District.district_name_guj AS district
                    FROM Account_Head
                    INNER JOIN Village
                        ON Village.village_id = Account_Head.village_id
                    INNER JOIN Taluka
                    ...
                    INNER JOIN District
                        ON District.district_id = Taluka.district_id
                    WHERE Account_Head.account_id = ?;
                    SELECT
                        Account_Balance.account_id AS sos_id,
                        Account_Balance.sub_account_id AS mem_id,
                        DATE_FORMAT(Account_Balance.insurance_date,'%d/%m/%Y') AS insurance_start_date,
                        DATE_FORMAT(Account_Balance.insurance_due_on_date,'%d/%m/%Y') AS insurance_end_date,
                        DATE_FORMAT(Account_Balance.birth_date,'%d/%m/%Y') AS birth_date,
                        DATEDIFF(Account_Balance.issue_date, Account_Balance.birth_date) AS ndays,
                        Account_Balance.issue_date AS issue_date,
                        Account_Balance.birth_wt AS cow_w,
                        Account_Balance.insurance_tag_no AS kadi_num,
                        Sub_Account.sub_account_name_guj AS sos_name,
                        Cow_Cast.cow_cast_name_guj AS cow_cast
                    FROM Account_Balance
                    INNER JOIN Sub_Account
                        ON Account_Balance.account_id = ? AND Account_Balance.sub_account_id = Sub_Account.sub_account_id
                    INNER JOIN Cow_Cast
                        ON Account_Balance.cow_cast_id = Cow_Cast.cow_cast_id
                    WHERE   Account_Balance.issue_date != '0000-00-00' AND Account_Balance.birth_date != '0000-00-00'
                        AND Account_Balance.issue_date >= ? AND Account_Balance.issue_date <= ?
                    ORDER BY Account_Balance.sub_account_id;
                    SELECT * FROM Kotha_Details WHERE account_id = ? ORDER BY sub_account_id ASC, entry_id ASC;
                `;
            }
            else
            {
                sql_arr = [data.account_id, data.account_id, data.sub_account_id_list, data.from_date, data.to_date ,data.account_id, data.sub_account_id_list];
                sql = `
                    SET NAMES UTF8;
                    SELECT
                        Account_Head.account_id AS sos_id,
                    ...
                    FROM Account_Balance
                    INNER JOIN Sub_Account
                        ON Account_Balance.account_id = ? AND Account_Balance.sub_account_id IN (?) AND Account_Balance.sub_account_id = Sub_Account.sub_account_id
                    INNER JOIN Cow_Cast
                        ON Account_Balance.cow_cast_id = Cow_Cast.cow_cast_id
                    WHERE   Account_Balance.issue_date != '0000-00-00' AND Account_Balance.birth_date != '0000-00-00'
                        AND Account_Balance.issue_date >= ? AND Account_Balance.issue_date <= ?
                    ORDER BY Account_Balance.sub_account_id;
                    SELECT * FROM Kotha_Details WHERE account_id = ? AND sub_account_id IN (?) ORDER BY sub_account_id ASC, entry_id ASC;
                `;
            }
        }
        else
        {
            sql_arr = [data.from_date, data.to_date, data.from_date, data.to_date, data.from_date, data.to_date];
            sql = `
                SET NAMES UTF8;
                SELECT
                    Account_Head.account_id AS sos_id,
                ...
                INNER JOIN District
                    ON District.district_id = Taluka.district_id
                WHERE Account_Head.account_id IN (SELECT account_id FROM Account_Balance WHERE issue_date >= ? AND issue_date <= ?);
                SELECT
                    Account_Balance.account_id AS sos_id,
                    Account_Balance.sub_account_id AS mem_id,
                    DATE_FORMAT(Account_Balance.insurance_date,'%d/%m/%Y') AS insurance_start_date,
                ...
                WHERE   Account_Balance.issue_date != '0000-00-00' AND Account_Balance.birth_date != '0000-00-00'
                    AND Account_Balance.issue_date >= ? AND Account_Balance.issue_date <= ?
                ORDER BY Account_Balance.sub_account_id;
                SELECT * FROM Kotha_Details WHERE sub_account_id IN (SELECT sub_account_id FROM Account_Balance WHERE issue_date >= ? AND issue_date <= ?) 
                ORDER BY sub_account_id ASC, entry_id ASC;
            `;
        }
    }
    else if (data.account_id)
    {
        if(data.select_all)
        {
            sql_arr = [data.account_id, data.account_id, data.account_id];
            sql = `
                SET NAMES UTF8;
                SELECT
                    Account_Head.account_id AS sos_id,
                ...
                SELECT
                    ...
                    Cow_Cast.cow_cast_name_guj AS cow_cast
                FROM Account_Balance
                INNER JOIN Sub_Account ON Account_Balance.account_id = ? AND Account_Balance.sub_account_id = Sub_Account.sub_account_id
                INNER JOIN Cow_Cast ON Account_Balance.cow_cast_id = Cow_Cast.cow_cast_id
                WHERE Account_Balance.issue_date != '0000-00-00' AND Account_Balance.birth_date != '0000-00-00'
                ORDER BY Account_Balance.sub_account_id;
                SELECT * FROM Kotha_Details WHERE account_id = ? ORDER BY sub_account_id ASC, entry_id ASC;
            `;
        ...
        {
            sql_arr = [data.account_id, data.account_id, data.sub_account_id_list, data.account_id, data.sub_account_id_list];
            sql = `
                SET NAMES UTF8;
                SELECT
                    Account_Head.account_id AS sos_id,
                ...
                SELECT
                    ...
                    Cow_Cast.cow_cast_name_guj AS cow_cast
                FROM Account_Balance
                INNER JOIN Sub_Account ON Account_Balance.account_id = ? AND Account_Balance.sub_account_id IN (?) AND Account_Balance.sub_account_id = Sub_Account.sub_account_id
                INNER JOIN Cow_Cast ON Account_Balance.cow_cast_id = Cow_Cast.cow_cast_id
                WHERE Account_Balance.issue_date != '0000-00-00' AND Account_Balance.birth_date != '0000-00-00'
                ORDER BY Account_Balance.sub_account_id;
                SELECT * FROM Kotha_Details WHERE account_id = ? AND sub_account_id IN (?) ORDER BY sub_account_id ASC, entry_id ASC;
            ...
            if (err) {
                ...
                connection.query(sql, sql_arr, (err, results) => {
                    ...
                    if (err) {
                        ...
                        var account_info = results[1];
                        var sub_account_info = results[2];
                        var kotha_details = results[3];
                        ...
                        console.log("-----------------------------------------------------------------------------------");
                        console.log("-----------------------------------------------------------------------------------");
                        console.log("SUB ACCOUNT INFO : ");
                        console.log(sub_account_info);
                        console.log("-----------------------------------------------------------------------------------");
                        console.log("-----------------------------------------------------------------------------------");
                        console.log("Kotha Details : ");
                        console.log(kotha_details);
                        console.log("-----------------------------------------------------------------------------------");
                        */

                        var sub_account_info_dict = {};
                        var kotha_details_dict = {};
                        var account_info_dict = {};
                        var sub_account, i, sub_account_id, sub_key, kotha_details_entry, issue_date, account;
    
                        var datarows = [];
    
                        for(i = 0; i < sub_account_info.length; i++) {
                            sub_account = sub_account_info[i];
                            sub_account_info_dict[sub_account.mem_id] = sub_account;
                        }
    
                        for(i = 0; i < kotha_details.length; i++) {
                            sub_account_id = kotha_details[i].sub_account_id;
                            if (!kotha_details_dict.hasOwnProperty(sub_account_id))
                                kotha_details_dict[sub_account_id] = [];
                            kotha_details_dict[sub_account_id].push(kotha_details[i]);
                        }

                        for(i = 0; i < account_info.length; i++) {
                            account = account_info[i];
                            account_info_dict[account.sos_id] = account;
                        }
    
                        var datarowsEntry = {}, ndays, count, start_month, temp_data, temp_entry, current_date;
                        
                        /*
                        console.log("-----------------------------------------------------------------------------------");
                        console.log("ACCOUNT DICT : ");
                        console.log(account_info_dict);
                        console.log("-----------------------------------------------------------------------------------");
                        console.log("-----------------------------------------------------------------------------------");
                        console.log("SUB ACCOUNT DICT : ");
                        console.log(sub_account_info_dict);
                        console.log("-----------------------------------------------------------------------------------");
                        console.log("-----------------------------------------------------------------------------------");
                        console.log("Kotha Details DICT : ");
                        console.log(kotha_details_dict);
                        console.log("-----------------------------------------------------------------------------------");
                        */
    
                        for(sub_key in sub_account_info_dict) {
                            sub_account = sub_account_info_dict[sub_key];
                            if(!account_info_dict.hasOwnProperty(sub_account.sos_id))
                                continue;
                            if(!kotha_details_dict.hasOwnProperty(sub_key))
                                continue;
                            account = account_info_dict[sub_account.sos_id];
                            datarowsEntry = {};
                            datarowsEntry["data"] = [];
                            ndays = sub_account["ndays"];
                            issue_date = sub_account["issue_date"];
                            if (ndays < 0)
                                continue;
                            start_month = Math.floor(ndays / 30) + 1;
                            count = 1;
                            current_date = new Date(issue_date);
                            temp_data = [];
                            for(var j = 0; j < kotha_details_dict[sub_key].length; j++) {
                                kotha_details_entry = kotha_details_dict[sub_key][j];
                                if(count >= start_month) {
                                    temp_entry = {
                                        num: getGujNum(kotha_details_entry.entry_id),
                                        date: getGujDate(current_date),
                                        item1: kotha_details_entry.item1,
                                        item2: kotha_details_entry.item2,
                                        item3: kotha_details_entry.item3,
                                        item4: kotha_details_entry.item4
                                    };
                                ...
                                temp_data.push(temp_entry);
                                count += 1;
                            }
                            datarowsEntry["data"] = temp_data;
                            datarowsEntry["sos_id"] = getGujNum(account["sos_id"]) || "";
                            datarowsEntry["sos_name"] = account["sos_name"] || "";
                            datarowsEntry["mem_id"] = getGujNum(sub_account["mem_id"]) || "";
                            datarowsEntry["mem_name"] = sub_account["mem_name"] || "";
                            datarowsEntry["village"] = account["village"] || "";
                            datarowsEntry["taluka"] = account["taluka"] || "";
                            datarowsEntry["district"] = account["district"] || "";
                            datarowsEntry["cow_cast"] = sub_account["cow_cast"] || "";
                            datarowsEntry["birth_date"] = getGujNum(sub_account["birth_date"]) || "";
                            datarowsEntry["insurance_start_date"] = getGujNum(sub_account["insurance_start_date"]) || "";
                            datarowsEntry["insurance_end_date"] = getGujNum(sub_account["insurance_end_date"]) || "";
                            datarowsEntry["cow_w"] = getGujNum(sub_account["cow_w"]) || "";
                            datarowsEntry["kadi_num"] = getGujNum(sub_account["kadi_num"]) || "";
    
                            datarows.push(datarowsEntry);
    
                        }
                        var dataobject = {
                            datarows
                        };
                        var template = "kotha-report";
                        reportGenerator(dataobject, template, (err, resheaders) => {
                            if (err) {
                                console.log(err);
                                res.send({
...
router.get('/jvbook', middleware.loggedin_as_superuser, (req, res) => {
    var data = req.query;
    var settings = {
        text_align_right: [3, 4],
        text_align_center: [1],
    ...
    var headers = ["Date", "Narration", "Credit", "Debit"];
    var report_title = "Journal Voucher Book";
    var report_information = `
        <tr style="background-color: white">
            <td></td>
        </tr>
    ...
    getConnection((err, connection) => {
        if (err) {
            ...
            sql = `
                SELECT
                    DATE_FORMAT(J.jv_date,'%d/%m/%Y') AS jdate,
                    J.amount AS total_amount,
                    J.narration AS main_narration,
                    JD.narration AS sub_narration,
                    JD.jv_serial_number AS jd_sr_num,
                    JD.account_id AS aid,
                    JD.sub_account_id AS sid,
                    JD.dr_amount AS dr,
                    JD.cr_amount AS cr
                FROM JV J
                INNER JOIN JV_Details JD
                    ON J.jv_number = JD.document_number
                WHERE J.jv_date >= ? AND J.jv_date <= ?
                ORDER BY J.document_number ASC, JD.jv_serial_number ASC;
            `;
            connection.query(sql, sql_arr, (err, results) => {
                connection.release();
                if (err) {
                    ...
                    if (results.length <= 0) {
                        ...
                        var data_entry = [], jvitem, final_narration = "", single_entry, s_cr_global = 0.00, s_dr_global = 0.00, cr = 0.00, dr = 0.00;

                        for(jvitem of results) {
                            cr = parseFloat(jvitem.cr);
                            dr = parseFloat(jvitem.dr);

                            final_narration = "";
                            if (jvitem.main_narration)
                                final_narration += `${jvitem.main_narration}<br style="display: block; margin-bottom: 0em;">`;
                            if (jvitem.sub_narration)
                                final_narration += `${jvitem.sub_narration}<br style="display: block; margin-bottom: 0em;">`;
                            final_narration += `${jvitem.sid}/(${jvitem.aid})`;

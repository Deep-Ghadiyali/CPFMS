...
router.get('/kothadetails/:accountid/:subaccountid', middleware.loggedin_as_superuser, (req, res) => {
    var account_id = String(req.params.accountid);
    var sub_account_id = String(req.params.subaccountid);
    getConnection((err, conncetion) => {
        if (err) {
            console.log(err);
            res.send({ status: false });
            ...
            var sql_arr = [account_id, sub_account_id, account_id, sub_account_id];
            var sql = `
                SET NAMES UTF8;
                SELECT item1, item2, item3, item4 FROM Kotha_Details WHERE account_id = ? AND sub_account_id = ? ORDER BY entry_id;
                SELECT DATE_FORMAT(issue_date,'%Y-%m-%d') AS issue_date FROM Account_Balance WHERE account_id = ? AND sub_account_id = ?;
            `;
            conncetion.query(sql, sql_arr, (err, results) => {
                if (err) {
                    conncetion.release();
                    console.log(err);
                    res.send({ status: false });
                    ...
                    if(results[1].length > 0)
                        res.send({
                            status: true,
                            data: results[1],
                            issue_date: results[2][0].issue_date
                        });
                    else
                        res.send({ status: false });
                }
...
router.get('/kothamembername/:id', middleware.loggedin_as_superuser, (req, res) => {
    getConnection((err, conncetion) => {
        if (err) {
            console.log(err);
            res.send({
            ...
            var sql = "SELECT sub_account_name FROM Sub_Account WHERE sub_account_id = ?; SELECT DATE_FORMAT(issue_date,'%Y-%m-%d') AS issue_date FROM Account_Balance WHERE sub_account_id = ?;";
            conncetion.query(sql, [req.params.id, req.params.id], (err, results) => {
                conncetion.release();
                if (err) {
                    console.log(err);
                    ...
                    if(results[1].length >= 1)
                        issue_date = results[1][0].issue_date;
                    res.send({
                        status: true,
                        data: results[0][0],
                        issue_date

        ...
        if (err) {
            ...
            connection.query(sql1, (err, results) => {
                if (err) {
                    ...
                    var sql2 = "SET NAMES UTF8; SELECT * FROM `Account_Head` LIMIT ? , ?; SELECT village_id FROM Village;";
...
router.get('/search', middleware.loggedin_as_superuser, (req, res) => {
    getConnection((err, connection) => {
        if (err) {
            ...
            var sql = "SET NAMES UTF8; SELECT * FROM Account_Head";
            var flag = false;
...
router.post('/', middleware.loggedin_as_superuser, (req, res) => {
    getConnection((err, connection) => {
        if (err) {
            ...
            var { account_id, account_name, account_name_guj, account_type, is_society, village_id } = req.body;
            account_id = account_id.trim();
            var sql = 'SET NAMES UTF8; INSERT INTO `Account_Head` (`account_id`, `account_name`, `account_name_guj`, `account_type`, `is_society`, `village_id`) VALUES (?, ?, ?, ?, ?, ?)'
            connection.query(sql, [account_id, account_name, account_name_guj, account_type, is_society, village_id], (err, result) => {
                connection.release();
                if (err) {
                    console.log(err);
...
router.post('/edit', middleware.loggedin_as_admin, (req, res) => {
    getConnection((err, connection) => {
        if (err) {
            ...
            var { account_id, account_name, account_name_guj, account_type, is_society, village_id } = req.body;
            account_id = account_id.trim();
            var sql = "SET NAMES UTF8; UPDATE `Account_Head` SET `account_name` = ?, `account_name_guj` = ?, `account_type` = ?, `is_society` = ?, `village_id` = ? WHERE `Account_Head`.`account_id` = ? ";
            connection.query(sql, [account_name, account_name_guj, account_type, is_society, village_id, account_id], (err, results) => {
                connection.release();
                if (err) {

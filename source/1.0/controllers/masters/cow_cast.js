        ...
        if (err) {
            ...
            connection.query(sql1, (err, results) => {
                if (err) {
                    ...
                    var sql2 = "SET NAMES UTF8; SELECT * FROM `Cow_Cast` LIMIT ? , ?";
...
router.get('/search', middleware.loggedin_as_superuser, (req, res) => {
    getConnection((err, connection) => {
        if (err) {
            ...
            var sql = "SET NAMES UTF8; SELECT * FROM Cow_Cast";
            var flag = false;
            var arr = ['searchtext'];
            for (key in ob) {
...
router.post('/', middleware.loggedin_as_superuser, (req, res) => {
    getConnection((err, connection) => {
        if (err) {
            ...
            var { cow_cast_id, cow_cast_name, cow_cast_name_guj } = req.body;
            cow_cast_id = cow_cast_id.trim();
            var sql = 'SET NAMES UTF8; INSERT INTO `Cow_Cast` (`cow_cast_id`, `cow_cast_name`, `cow_cast_name_guj`) VALUES (?, ?, ?)'
            connection.query(sql, [cow_cast_id, cow_cast_name, cow_cast_name_guj],
                (err, result) => {
                    connection.release();
                    if (err) {
                        console.log(err);
...
router.post('/edit', middleware.loggedin_as_admin, (req, res) => {
    getConnection((err, connection) => {
        if (err) {
            ...
            var { cow_cast_id, cow_cast_name, cow_cast_name_guj } = req.body;
            cow_cast_id = cow_cast_id.trim();
            var sql = "SET NAMES UTF8; UPDATE `Cow_Cast` SET `cow_cast_name` = ?, `cow_cast_name_guj` = ? WHERE `Cow_Cast`.`cow_cast_id` = ? ";
            connection.query(sql, [cow_cast_name, cow_cast_name_guj, cow_cast_id], (err, results) => {
                connection.release();
                if (err) {

        ...
        if (err) {
            ...
            connection.query(sql1, (err, results) => {
                if (err) {
                    ...
                    var sql2 = "SET NAMES UTF8; SELECT * FROM `District` LIMIT ? , ?";
...
router.get('/search', middleware.loggedin_as_superuser, (req, res) => {
    getConnection((err, connection) => {
        if (err) {
            ...
            var sql = "SET NAMES UTF8; SELECT * FROM District";
            var flag = false;
            var arr = ['searchtext'];
            for (key in ob) {
...
router.post('/', middleware.loggedin_as_superuser, (req, res) => {
    getConnection((err, connection) => {
        if (err) {
            ...
            var { district_id, district_name, district_name_guj } = req.body;
            district_id = district_id.trim();
            var sql = 'SET NAMES UTF8; INSERT INTO `District` (`district_id`, `district_name`, `district_name_guj`) VALUES (?, ?, ?)';
            connection.query(sql, [district_id, district_name, district_name_guj],
                (err, result) => {
                    connection.release();
                    if (err) {
                        console.log(err);
...
router.post('/edit', middleware.loggedin_as_admin, (req, res) => {
    getConnection((err, connection) => {
        if (err) {
            ...
            var { district_id, district_name, district_name_guj } = req.body;
            district_id = district_id.trim();
            var sql = " UPDATE `District` SET `district_name` = ?, `district_name_guj` = ? WHERE `District`.`district_id` = ? ";
            connection.query(sql, [district_name, district_name_guj, district_id], (err, results) => {
                connection.release();
                if (err) {

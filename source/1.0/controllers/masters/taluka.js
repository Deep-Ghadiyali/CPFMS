        ...
        if (err) {
            ...
            connection.query(sql1, (err, results) => {
                if (err) {
                    ...
                    var sql2 = "SET NAMES UTF8; SELECT * FROM Taluka INNER JOIN District ON Taluka.district_id = District.district_id LIMIT ? , ?;SELECT * FROM District";
...
router.get('/search', middleware.loggedin_as_superuser, (req, res) => {
    getConnection((err, connection) => {
        if (err) {
            ...
            var sql = "SET NAMES UTF8; SELECT * FROM Taluka INNER JOIN District ON Taluka.district_id = District.district_id";
            var flag = false;
            var arr = [];
            if (ob.district_id) {
...
router.post('/', middleware.loggedin_as_superuser, (req, res) => {
    getConnection((err, connection) => {
        if (err) {
            ...
            var { taluka_id, taluka_name, taluka_name_guj, district_id } = req.body;
            taluka_id = taluka_id.trim();
            var sql = 'SET NAMES UTF8; INSERT INTO `Taluka` (`taluka_id`, `taluka_name`, `taluka_name_guj`, `district_id`) VALUES (?, ?, ?, ?)';
            connection.query(sql, [taluka_id, taluka_name, taluka_name_guj, district_id], (err, result) => {
                connection.release();
                if (err) {
                    console.log(err);
...
router.post('/edit', middleware.loggedin_as_admin, (req, res) => {
    getConnection((err, connection) => {
        if (err) {
            ...
            var { taluka_id, taluka_name, taluka_name_guj, district_id } = req.body;
            taluka_id = taluka_id.trim();
            var sql = "SET NAMES UTF8; UPDATE `Taluka` SET `taluka_name` = ?, `taluka_name_guj` = ?, `district_id` = ? WHERE `Taluka`.`taluka_id` = ? ";
            connection.query(sql, [taluka_name, taluka_name_guj, district_id, taluka_id], (err, results) => {
                connection.release();
                if (err) {

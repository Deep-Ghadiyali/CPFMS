        ...
        if (err) {
            ...
            connection.query(sql1, (err, results) => {
                if (err) {
                    ...
                    var sql2 = "SET NAMES UTF8; SELECT * FROM Village INNER JOIN Taluka ON Village.taluka_id = Taluka.taluka_id LIMIT ? , ?;SELECT * FROM Taluka";
...
router.get('/search', middleware.loggedin_as_superuser, (req, res) => {
    getConnection((err, connection) => {
        if (err) {
            ...
            var sql = "SET NAMES UTF8; SELECT * FROM Village INNER JOIN Taluka ON Village.taluka_id = Taluka.taluka_id";
            var flag = false;
            var arr = [];
            if (ob.taluka_id) {
...
router.post('/', middleware.loggedin_as_superuser, (req, res) => {
    getConnection((err, connection) => {
        if (err) {
            ...
            var { village_id, village_name, village_name_guj, taluka_id } = req.body;
            village_id = village_id.trim();
            var sql = 'SET NAMES UTF8; INSERT INTO `Village` (`village_id`, `village_name`, `village_name_guj`, `taluka_id`) VALUES (?, ?, ?, ?)'
            connection.query(sql, [village_id, village_name, village_name_guj, taluka_id], (err, result) => {
                connection.release();
                if (err) {
                    console.log(err);
...
router.post('/edit', middleware.loggedin_as_admin, (req, res) => {
    getConnection((err, connection) => {
        if (err) {
            ...
            var { village_id, village_name, village_name_guj, taluka_id } = req.body;
            village_id = village_id.trim();
            var sql = "SET NAMES UTF8; UPDATE `Village` SET `village_name` = ?, `village_name_guj` = ?, `taluka_id` = ? WHERE `Village`.`village_id` = ? ";
            connection.query(sql, [village_name, village_name_guj, taluka_id, village_id], (err, results) => {
                connection.release();
                if (err) {

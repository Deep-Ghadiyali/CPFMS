    ...
    getConnection((err, connection) => {
        if (err) {
            ...
            var sql = `
                ...
                SELECT activity_id FROM Activity_Master;
            ...
            connection.query(sql, (err, results) => {
                ...
                if (err) {
                    ...
                    for (var account_id of results[0])
                    {
                        account_head_list.push(account_id.account_id);
                    }
                    res.render("transactions/activity_jv/addform", {
                        account_head: results[0],
                        activity_id: results[1],
                        document_number: parseInt(results[2][0].maxcount) + 1,
                        today_date: date,
                        account_head_list: account_head_list
                    });
                }
            });
        }
